...
public class BlogNoteController {
	...
	public void downloadBlog(String blogIds, HttpServletResponse response, HttpServletRequest request)
			throws IOException {
		...
		if (blogIdArr != null && blogIdArr.length == 1) {
			...
			String fileName = blogNote.getTitle() + "_"
					+ DateUtils.getDateToStringByPattern(new Date(), "yyyyMMddHHmmss");
			response.reset();
			response.setContentType("application/msword;charset=utf-8");
			response.setHeader("Content-Disposition",
					"attachment;filename=" + new String(fileName.getBytes("GBK"), "ISO8859-1") + ".doc");
	 ...
	 * @throws IOException
	 */
	private void downloadSingleBlog(OutputStream out, BlogNote blogNote, HttpServletResponse response)
			throws IOException {
		Template template = getTemplate();
		if (template != null) {
			Map<String, Object> dataMap = new HashMap<String, Object>();
			dataMap.put("createTime",
					DateUtils.getDateToStringByPattern(blogNote.getCreateTime(), "YYYY年MM月dd日  hh:mm"));
